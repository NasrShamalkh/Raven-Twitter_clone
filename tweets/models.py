...
class Tweet(models.Model):
    ...
    user = models.ForeignKey(User, related_name='tweets', on_delete=models.CASCADE) # each user can have multiple tweets
    ...
    ###-------------------------------###
    likes = models.ManyToManyField(User, related_name='liked_tweets', through=TweetLike_relation)
    retweets = models.ManyToManyField(User, related_name='retweeted', through=TweetRetweet_relation)
    saves = models.ManyToManyField(User, related_name='saved_tweets', through=TweetSave_relation)
    ###-------------------------------###
    timestamp = models.DateTimeField(auto_now_add=True)
...
class Reply(models.Model):
    ...
    user = models.ForeignKey(User, related_name='user_replies', on_delete=models.CASCADE) # each user can have multiple replies
    content = models.CharField(max_length=280) # max length of a tweet / reply is 280 characters
    media = models.BooleanField(default=False)
    media_url = models.URLField(blank=True, null=True)
    likes = models.ManyToManyField(User, related_name='liked_replies', through=ReplyLike_relation)
    timestamp = models.DateTimeField(auto_now_add=True)
